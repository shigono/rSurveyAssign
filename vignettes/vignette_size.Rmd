...
["rSurveyAssign: run simulations of assignments for web surveys"](vignette.html)を参照して下さい。
...
以下では、母集団メンバーを$i (=1,\ldots,I)$, カテゴリを$j(=1,\ldots,J)$, カテゴリ$j$のスロットを$k (=1,\ldots,K[j])$と表記します。

データのサイズ($I, J, K[1],...,K[J]$)に制限はありません。$I$ はできるだけ大きくしたほうがよいでしょう。

母集団データの例として、`popdata`が用意されています。
`popdata`は、サイズ10000の有限集団のデータを持つデータフレームで、
カテゴリ数は3, スロット数は
各カテゴリについて10です(すなわち、$I=10000, J=3, K[1]=K[2]=K[3]=10$)。
...
- `mbCAT`: カテゴリ割付可能性。
    ...
    - $i$行$j$列の値は、「母集団メンバー$i$にとってカテゴリ$j$は割付可能か」
      を表し、$0$は割付不能, $1$は割付不能を表します。
    - 欠損は許容されません。
- `lSLOT`: スロット割付可能性。
    - $J$個の要素からなるリスト。$j$番目の要素は、$I$行$K[j]$列の整数行列。
    - 要素$j$の行列の$i$行$k$列の値は、「母集団メンバー$i$にとって   カテゴリ$j$のスロット$k$は割付可能か」を表し、$0$は割付不能, $1$は割付不能を表します。
...
- `lSLOT_REQUEST`: 各スロットの目標対象者数。
    - $J$個の要素を持つリスト。$j$番目の要素は長さ$K[j]$の整数ベクトル。
    - $j$番目の要素であるベクトルの$k$番目の要素は、カテゴリ$j$のスロット$k$に割り付ける対象者数の下限を表します。欠損は許容されません。
- `nCAT_MAX`: 割付カテゴリ数の上限。
- `sCAT_TYPE`, `sCAT_FILTER`, `sCAT_ORDER`, `sCAT_EXCLUDE`: カテゴリ割付のタイプ、絞り込み条件、順序付け条件、除外条件。
["rSurveyAssign: run simulations of assignments for web surveys"](vignette.html)
を参照して下さい。
- `nSLOT_MAX`: 割付スロット数の上限。
- `sSLOT_TYPE`, `sSLOT_FILTER`, `sSLOT_ORDER`, `sSLOT_EXCLUDE`: スロット割付のタイプ、絞り込み条件、順序付け条件、除外条件。
["rSurveyAssign: run simulations of assignments for web surveys"](vignette.html)
を参照して下さい。
...
- 回答負荷の観点から、ひとりの対象者に割り付けるカテゴリは2つまで、スロットは2つまでとします。
- カテゴリの割付方法として(adaptive,open,random,none)を使用し、スロットの割付方法として(adaptive,all,shortnum,allclosed)を使用します。
